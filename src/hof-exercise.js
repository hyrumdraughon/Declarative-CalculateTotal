...
const prices = items => items.map(currentValue => currentValue.price); 
...
const selectTaxable = items => items.filter(currentValue => currentValue.taxable); 

// applyTax: (prices: [number], tax: number) -> [number]
const applyTax = (prices, tax) => prices.map(currentValue => currentValue * tax); 
